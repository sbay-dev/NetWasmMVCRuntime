...
// Activate scripts + styles from innerHTML (which doesn't run them natively)
// External scripts (src) are loaded sequentially first, then inline scripts execute
// Style tags are promoted to <head> for reliable application
function activateScripts(container) {
    // Promote <style> tags to <head> (remove old view styles first)
    ...
    // Process <script> tags
    const dead = [...container.querySelectorAll('script')];
    const externals = [], inlines = [];
    for (const old of dead) {
        ...
        old.remove();
        if (live.src) externals.push(live); else inlines.push(live);
    }
    let i = 0;
    function next() {
        if (i < externals.length) {
            ...
            for (const s of inlines) container.appendChild(s);
        }
    }
    next();
}
