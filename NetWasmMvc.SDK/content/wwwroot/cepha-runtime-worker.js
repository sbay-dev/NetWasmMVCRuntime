...
self.onmessage = async (e) => {
    ...
    switch (type) {
        ...
        case 'hub-invoke':
            ...
            break;
        case 'fetch': {
            // API fetch: route through MVC engine via HandleRequest
            const { id: fetchId, method: fetchMethod, path: fetchPath, body: fetchBody } = e.data;
            if (_exports) {
                try {
                    const responseJson = await _exports.Cepha.JsExports.HandleRequest(
                        fetchMethod || 'GET', fetchPath, null, fetchBody || null);
                    self.postMessage({ type: 'fetch-result', id: fetchId, response: responseJson });
                } catch (err) {
                    self.postMessage({ type: 'fetch-result', id: fetchId, response: JSON.stringify({
                ...
                self.postMessage({ type: 'fetch-result', id: fetchId, response: JSON.stringify({
                    ...
                    body: JSON.stringify({ error: 'Runtime not ready' })
                })});
            }
            break;
        }
