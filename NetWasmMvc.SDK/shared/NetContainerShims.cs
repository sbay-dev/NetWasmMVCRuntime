...
namespace NetContainer.Ref.Orchestrator
{
    ...
    internal sealed class BrowserRefOrchestrator : IRefOrchestratorService
    {
            ...
            Guest.RefGuestOptions options, CancellationToken ct = default)
        {
            ...
            // Honest: no real guest is booting — log only what's true
            ...
            guest.AppendLog($"");
            guest.AppendLog($"⚠ No real hypervisor in browser-wasm mode.");
            guest.AppendLog($"  Connect CephaKit server-mode for real QEMU guests.");
            guest.AppendLog($"  Or integrate v86 (copy-on-write/npm) for in-browser x86 emulation.");
...
namespace NetContainer.Ref.Services
{
    ...
    // Shell is not available in browser-wasm mode — no real guest OS to interact with.
    // Returns honest "not available" for all commands.
    // For real shell interaction, use CephaKit server-mode (QEMU over WebSocket).
    internal sealed class BrowserShellService : IShellService
    {
        private const string NotAvailable =
            "⚠ Shell not available in browser-wasm mode.\n" +
            "No real guest OS is running — there is no kernel, no filesystem, no processes.\n" +
            "To interact with a real guest, connect via CephaKit server-mode.";
        ...
        public Task<ShellResult> RunAsync(string command, int timeoutMs = 5000,
            CancellationToken ct = default)
            => Task.FromResult(new ShellResult { ExitCode = 1, StdOut = NotAvailable });
